...
# 鏍煎紡: { name: 涓枃鍚?甯哥敤鍚?, arxiv_name: ArXiv浣滆€呭悕(Last, First), github_user: GitHub鐢ㄦ埛鍚嶆垨org, twitter_id: Twitter鐢ㄦ埛ID鏁板瓧, weibo_uid: 寰崥UID }
BIG_NAMES = [
 # === 椤剁骇鐮旂┒鏈烘瀯/浼佷笟棰嗚 ===
 ...
 {name: 鏉庨椋?(Fei-Fei Li), arxiv_name: Li, Fei-Fei, github_user: , twitter_id: , weibo_uid: },
 {name: 鍚存仼杈?(Andrew Ng), arxiv_name: Ng, Andrew, github_user: andrewng, twitter_id: , weibo_uid: },
 ...
 {name: 姊佹枃閿?(DeepSeek), arxiv_name: , github_user: deepseek-ai, twitter_id: , weibo_uid: },
 ...
 # === 涓浗AI鍦?===
 {name: 寮犱簹鍕?,               arxiv_name: Zhang, Ya-Qin,           github_user: ",                   twitter_id: ", weibo_uid: "},
    {name: 鍞愭澃 (鏅鸿氨/ChatGLM),   arxiv_name: Tang, Jie,              github_user: THUDM,              twitter_id: ", weibo_uid: "},
    {name: 鐜嬪皬宸?(鐧惧窛鏅鸿兘),     arxiv_name: ",                       github_user: baichuan-inc,       twitter_id: ", weibo_uid: "},
    {name: 鏉庡紑澶?(闆朵竴涓囩墿),     arxiv_name: Lee, Kai-Fu,            github_user: ",                   twitter_id: ", weibo_uid: "},
    {name: 鍛ㄩ缚绁?(360),         arxiv_name: ",                       github_user: ",                   twitter_id: ", weibo_uid: "},
    {name: 榛勪粊鍕?(Jensen Huang), arxiv_name: ",                       github_user: ",                   twitter_id: ", weibo_uid: "},
...
def fetch_arxiv_papers(arxiv_name, max_results=MAX_PER_SOURCE):
 ...
 url = (
 fhttp://export.arxiv.org/api/query
 ...
 papers = []
 cutoff = YESTERDAY.replace(tzinfo=None) # feedparser 鐨?published_parsed 鏄?naive UTC
...
def fetch_twitter_tweets(twitter_id, max_results=MAX_PER_SOURCE):
 "鏌ヨ Twitter 鐢ㄦ埛鏈€杩戞帹鏂囷紙闇€ bearer token锛?"
...
def build_feishu_card(all_results):
    "鏋勫缓椋炰功鍗＄墖娑堟伅"
    header_text = fAI鍦堝ぇ浣姩鎬佹棩鎶?| {TODAY}
    elements = []
    ...
    # 鎽樿琛?    summary_parts = []
    if total_papers:
        summary_parts.append(f馃搫 璁烘枃 {total_papers} 绡?)
 if total_github:
 summary_parts.append(f馃捇 GitHub {total_github} 鏉?)
    if total_twitter:
        summary_parts.append(f馃惁 Twitter {total_twitter} 鏉?)
 if total_weibo:
 summary_parts.append(f馃寪 寰崥 {total_weibo} 鏉?)
...
def send_to_feishu(card):
 ...
 print(鉁?椋炰功鎺ㄩ€佹垚鍔?)
            ...
            print(f鉂?椋炰功鎺ㄩ€佸け璐? {result})
            return False
    except Exception as e:
        print(f鉂?椋炰功鎺ㄩ€佸紓甯? {e})
        return False
